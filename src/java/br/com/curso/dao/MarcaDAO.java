...
public class MarcaDAO implements GenericDAO{
   ...
   @Override
    ...
    public Boolean inserir(Object objeto) {
        ...
        String sql = "insert into marcaveiculo (nomemarca, imagemmarca) values (?,?)";
        try{
            stmt = conexao.prepareStatement(sql);
            stmt.setString(1, oMarca.getNomeMarca());
            stmt.setString(2, oMarca.getImagemMarca());
            stmt.execute();
            conexao.commit();
            return true;
        } catch (Exception ex){
    ...
    public Boolean alterar(Object objeto) {
         Marca oMarca = (Marca) objeto;
       PreparedStatement stmt = null;
       String sql = "update marcaveiculo set nomemarca=?, imagemmarca=? where idmarca=?";
       try {
           stmt = conexao.prepareStatement(sql);
           stmt.setString(1, oMarca.getNomeMarca());
           stmt.setString(2, oMarca.getImagemMarca());
    ...
    public Boolean excluir(int numero) {
        ...
        String sql = "delete from marcaveiculo where idmarca=?";
        try {
            stmt = conexao.prepareStatement(sql);
            ...
            System.out.println("Problema ao excluir a Marca! Erro: "
                    +ex.getMessage());
            try {
                conexao.rollback();
            } catch (SQLException e) {
    ...
    public Object carregar(int numero) {
        ...
        ResultSet rs= null;
        Marca oMarca = null;
        String sql="select * from marcaveiculo where idmarca=?";
        ...
        try{
            ...
            while (rs.next()) {
                ...
                oMarca.setImagemMarca(rs.getString("imagemmarca"));
            }
            return oMarca;
        } catch (Exception ex) {
            System.out.println("Problema ao carregar Marca! Erro:"+ex.getMessage());
            return false;
        }
    }
    ...
    public List<Object> listar() {
       List<Object> resultado = new ArrayList<>();
        PreparedStatement stmt = null;
        ResultSet rs = null;
        String sql = "Select * from marcaveiculo order by idMarca";
        try {
            stmt = conexao.prepareStatement(sql);
            rs=stmt.executeQuery();
            while (rs.next()){
     ...
     public String listarJSON(){
        ...
        List<Object> resultado = new ArrayList<>();
        Marca oMarca = null;
        String sql = "select * from marcaveiculo";
        try {
            stmt = conexao.prepareStatement(sql);
            rs = stmt.executeQuery();
            strJson = "[";
            ...
            while(rs.next()){
                if (i>0) strJson+=",";
                strJson += "{\"idMarca\":"+rs.getInt("idmarca")+","
                        + "\"nomeMarca\":\""+rs.getString("nomemarca")+"\",";
                i++;
            }
            strJson += "]";
        } catch (Exception e){
            System.out.println("Problemas ao Listar Marca!Erro: " + e.getMessage());
            e.printStackTrace();
        }
